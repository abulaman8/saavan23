...
@api_view(['PUT'])
@event_head_required
def close_registration(request, id):
    try:
        event = Event.objects.get(id=id)
    except Event.DoesNotExist:
        return Response({"message": "event does not exist"}, status=status.HTTP_404_NOT_FOUND)
    ...
    event.open = False
    event.save()
    return Response({"message": "registration closed successfully"}, status=status.HTTP_200_OK)


@api_view(['PUT'])
@event_head_required
def open_registration(request, id):
    try:
        event = Event.objects.get(id=id)
    except Event.DoesNotExist:
        return Response({"message": "event does not exist"}, status=status.HTTP_404_NOT_FOUND)
    ...
    event.open = True
    event.save()
    return Response({"message": "registration opened successfully"}, status=status.HTTP_200_OK)
