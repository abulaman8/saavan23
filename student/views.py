        ...
        if application:
            ...
            for member in data['team_members']:
                try:
                    ...
                    return Response({'message': 'User does not exist'}, status=status.HTTP_404_NOT_FOUND)
                team_members.append(student)
            for member in application.team.members.all():
                member.events.remove(event)
            application.team.members.clear()
            for member in team_members:
                member.events.add(event)
                application.team.members.add(member)
                member.save()
            application.team.name = data.get('team_name', application.team.name)
            application.team.save()
            application.custom_data = data.get('custom_data', application.custom_data)
            application.save()
